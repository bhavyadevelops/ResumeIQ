  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
        ...
        <div className="md:col-span-4 bg-white border border-slate-100 rounded-2xl p-6 shadow-sm flex flex-col items-center justify-center relative overflow-hidden">
          ...
          <div className="text-center mt-5">
            <h4 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Compatibility Tier</h4>
        ...
        <div className="md:col-span-8 bg-white border border-slate-100 rounded-2xl p-6 shadow-sm flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-4">
              <span className="text-xs font-bold text-slate-400 uppercase tracking-wider">Feature: Recruiter Simulator</span>
            ...
            <p className="text-sm text-slate-700 font-medium italic leading-relaxed mb-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-3 bg-emerald-50/50 rounded-lg border border-emerald-100/30">
                <h5 className="text-xs font-bold text-emerald-800 uppercase tracking-wider mb-2 flex items-center gap-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span> Key Recruiter Strengths
                </h5>
                <ul className="text-xs text-slate-600 flex flex-col gap-1.5 list-disc pl-4 font-normal">
              ...
              <div className="p-3 bg-rose-50/50 rounded-lg border border-rose-100/30">
                <h5 className="text-xs font-bold text-rose-800 uppercase tracking-wider mb-2 flex items-center gap-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-rose-500"></span> Primary Concerns / Risks
                </h5>
                <ul className="text-xs text-slate-600 flex flex-col gap-1.5 list-disc pl-4 font-normal">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 flex flex-col gap-3">
          <div className="bg-white border border-slate-100 rounded-2xl p-5 shadow-sm">
            <h3 className="text-sm font-semibold text-slate-800 mb-4 border-b border-rose-10/50 pb-2">ATS Scoring Grid</h3>
              ...
              {categories.map((cat) => (
                ...
                >
                  <div className="flex flex-col gap-0.5 max-w-[70%]">
                    <span className="text-xs font-bold block truncate">{cat.title}</span>
                    <span className={`text-[10px] font-medium tracking-wide font-mono ${selectedCat === cat.key ? "text-indigo-300" : "text-slate-400"}`}>
                      Weight: {cat.weight}
                    </span>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className={`text-sm font-extrabold font-mono ${selectedCat === cat.key ? "text-white" : "text-slate-900"}`}>
                      {cat.val}%
                    </span>
                    <ChevronRight className={`w-3.5 h-3.5 ${selectedCat === cat.key ? "text-indigo-300" : "text-slate-400"}`} />
                  </div>
                </button>
              ))}
            </div>
        ...
        <div className="lg:col-span-7">
          <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm min-h-full flex flex-col justify-between">
            <div>
              <div className="flex items-center justify-between border-b border-slate-100 pb-3 mb-4">
                <h4 className="text-sm font-semibold text-slate-900 flex items-center gap-1.5">
                  <Award className="w-4 h-4 text-indigo-500" /> Explainable Diagnostics: {categories.find(c => c.key === selectedCat)?.title}
                </h4>
                <span className="text-xs font-black text-slate-900 bg-slate-100 border px-2 py-0.5 rounded font-mono">
              ...
              {/* Text Why */}
              <div className="mb-4">
                <span className="text-[10px] uppercase font-bold text-slate-400 tracking-wider block mb-1">Scoring Rationale</span>
                <p className="text-xs text-slate-700 bg-slate-50 p-3 rounded-lg border border-slate-100 border-dashed leading-relaxed">
                  {activeDetail.why}
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                  <span className="text-[10px] uppercase font-bold text-emerald-600 tracking-wider block mb-1.5">Contributing Strengths</span>
                  <ul className="text-xs text-slate-600 flex flex-col gap-1.5 font-normal">
                ...
                <div>
                  <span className="text-[10px] uppercase font-bold text-rose-500 tracking-wider block mb-1.5">Missing Elements</span>
                  <ul className="text-xs text-slate-600 flex flex-col gap-1.5 font-normal">
              ...
              <div className="border-t border-slate-100 pt-3">
                <span className="text-[10px] uppercase font-bold text-indigo-500 tracking-wider block mb-2">Prescribed Actions to Boost Score</span>
                <div className="flex flex-col gap-1.5">
                  {activeDetail.recommendations.map((rec, i) => (
                    <div key={i} className="text-xs text-slate-700 flex items-start gap-1.5 pb-1">
      ...
      <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm">
        <h3 className="text-sm font-semibold text-slate-900 mb-4 flex items-center gap-1.5 border-b border-slate-100 pb-2">
          <Award className="w-4 h-4 text-indigo-500" /> Applicant Benchmarking Percentiles
        </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          {[
            { label: "Core Skills Match", val: benchmarking.skillsPercentile, color: "bg-indigo-500" },
            { label: "Project Benchmarking", val: benchmarking.projectsPercentile, color: "bg-blue-500" },
            { label: "Work History Relevance", val: benchmarking.experiencePercentile, color: "bg-emerald-500" },
            { label: "ATS Formatting Compliance", val: benchmarking.formattingPercentile, color: "bg-teal-500" },
            { label: "Achievements Strength", val: benchmarking.achievementsPercentile, color: "bg-violet-500" },
          ].map((bench, idx) => (
            <div key={idx} className="bg-slate-50/50 p-4 rounded-xl border border-slate-100">
              <span className="text-[11px] font-bold text-slate-500 uppercase tracking-widest block mb-1 truncate">{bench.label}</span>
              <div className="flex items-baseline gap-1 mb-2">
                <span className="text-2xl font-black text-slate-800 font-mono">{bench.val}th</span>
                <span className="text-[10px] text-slate-400">Percentile</span>
              </div>
      ...
      <div className="bg-white border border-slate-100 rounded-2xl p-6 shadow-sm">
        <h3 className="text-sm font-semibold text-slate-900 mb-4 flex items-center gap-1.5 border-b border-slate-100 pb-2">
          <CheckCircle2 className="w-4 h-4 text-emerald-500" /> Resume Section Integrity Health
        </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {Object.entries(sectionAnalysis).map(([sectionName, info]) => {
            ...
            if (inf.status === "Missing") statusColor = "bg-rose-50 text-rose-700 border-rose-100";
            else if (inf.status === "Weak") statusColor = "bg-amber-50 text-amber-700 border-amber-100";
            else if (inf.status === "Overloaded") statusColor = "bg-blue-50 text-blue-700 border-blue-100";

            return (
              <div key={sectionName} className="p-4 rounded-xl border border-slate-100 bg-slate-50/40 flex flex-col justify-between">
